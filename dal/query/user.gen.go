...
func newUser(db *gorm.DB, opts ...gen.DOOption) user {
	...
	_user.WUsers = userHasManyWUsers{
...
func (a userHasManyWUsers) Where(conds ...field.Expr) *userHasManyWUsers {
	if len(conds) == 0 {
		return &a
	}
...
func (a userHasManyWUsers) WithContext(ctx context.Context) *userHasManyWUsers {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a userHasManyWUsers) Session(session *gorm.Session) *userHasManyWUsers {
	a.db = a.db.Session(session)
	return &a
}

func (a userHasManyWUsers) Model(m *model.User) *userHasManyWUsersTx {
	return &userHasManyWUsersTx{a.db.Model(m).Association(a.Name())}
}

type userHasManyWUsersTx struct{ tx *gorm.Association }

func (a userHasManyWUsersTx) Find() (result []*model.WUser, err error) {
	return result, a.tx.Find(&result)
}

func (a userHasManyWUsersTx) Append(values ...*model.WUser) (err error) {
...
func (a userHasManyWUsersTx) Replace(values ...*model.WUser) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userHasManyWUsersTx) Delete(values ...*model.WUser) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userHasManyWUsersTx) Clear() error {
	return a.tx.Clear()
}

func (a userHasManyWUsersTx) Count() int64 {
	return a.tx.Count()
}
